...
- [Admin course permissions](admin-course-permissions.md) — grantedBy bug: courses.map() was dropping fields; admin_course_permissions table for per-admin course enforcement; super_admin bypasses.
- [Device Security Admin permission](device-security-admin-permission.md) — access requires manage_device_security independently of manage_users; Super Admin bypasses; enforce UI and every API route.
