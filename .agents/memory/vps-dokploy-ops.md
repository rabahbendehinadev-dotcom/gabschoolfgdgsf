...
- New PostgreSQL schemas must use the API's existing additive, idempotent startup migration path; Replit Publish schema sync does not reach the external Dokploy database.
- **Why:** Solutions routes deployed successfully while all three module tables were absent, so both public and Admin list APIs returned 500.
- **How to apply:** export the Drizzle schema and add non-destructive `CREATE TABLE/INDEX IF NOT EXISTS` statements to `runMigrations`; never reset or overwrite VPS data.
